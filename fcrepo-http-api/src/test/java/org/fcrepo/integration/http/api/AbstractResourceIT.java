...
public abstract class AbstractResourceIT {
    ...
    protected static void concurrentSNSDataStreamIngest(final HttpUriRequest req)
        ...
        assertEquals(CREATED.getStatusCode(), firstTask.get().intValue());
        final int status = secondTask.get().intValue();
        if (status == CREATED.getStatusCode()) {
            // test Same Name Sibling node
            final String snSiblingUrl = req.getURI() + URLEncoder.encode("[2]", "UTF-8");
            assertEquals(404, getStatus(new HttpGet(snSiblingUrl)));
        } else {
