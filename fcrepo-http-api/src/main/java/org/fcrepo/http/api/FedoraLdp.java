...
public class FedoraLdp extends ContentExposingResource {
     ...
     */
    private void save(final FedoraResource resource, final String path) throws RepositoryException {
        synchronized (FedoraLdp.class) {
            final FedoraResource existingNode = nodeService.find(session, path);
            if (existingNode != null && !existingNode.getPath().equals(resource.getPath())) {
                LOGGER.error("Same Name Sibling violation {}: {}", existingNode.getPath(), resource.getPath());
                session.removeItem(resource.getPath());
                throw new ClientErrorException("Resource Already Exists: " + path, CONFLICT);
            }
            session.save();
        }
    }
